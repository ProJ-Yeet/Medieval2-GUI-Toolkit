...
| Minor Files | Rebel factions, religions, resources and character names; **Cultures** on a screen of its own, with each culture's settlements, fort, port ladder, watchtower and agents on four tabs, and **Duplicate** to make a new culture from one that works; and **Campaign constants**: `descr_campaign_db.xml` as a form typed off the file itself, with what the archive's tutorials say about the forts, piety and ransom settings |
