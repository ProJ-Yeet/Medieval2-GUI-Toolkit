...
| My changes | Everything you changed in a mod, recorded as you go with nothing to switch on. When an update overwrites the mod, port your changes back onto it change by change: each one says whether it applies cleanly, merges with the update, conflicts with it, or names something the update removed. Export the record as one file, and import one |
