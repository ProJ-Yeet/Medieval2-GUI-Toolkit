...
| the reasoning behind a backlog item | `merge/REFERENCE_GAPS.md` |
...
commit messages, `merge/PORT_MANIFEST.json` and `STATE.md` still resolve.
...
The forty open items in `merge/REFERENCE_GAPS.md` were classified by the user on
...
The `notes` fields in `merge/PORT_MANIFEST.json` that said "deferred to V3.1" and
...
- **M3 - `descr_events.txt`.** The historical events a campaign fires: dates,
  ...
  was explicitly kept in scope at triage - `merge/SYNC_LOG.md` records that what
...
| G2 | Region music | S | `mapquery` reads `descr_sounds_music_types.txt` already; the region form does not offer it. |
...
`merge/REFERENCE_GAPS.md` holds the full write-up of all fifty-one items - what
...
`merge/PORT_MANIFEST.json` when Phase 25 starts. `LuaAiAssistant`,
...
reasoning goes in `merge/` notes or the roadmap, not here; phase exit criteria
