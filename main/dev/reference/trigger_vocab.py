"""Build the trigger vocabulary from the sources, so it is data and not code.
    ...
    python dev/reference/trigger_vocab.py            # rebuild unittransfer/data/trigger_vocab.json
    python dev/reference/trigger_vocab.py --check    # report what would change, write nothing
    python dev/reference/trigger_vocab.py --audit    # what the reference tool's list gets wrong
...
def audit(vocab: dict) -> str:
    ...
    lines = [
        ...
        "",
        "_Generated by `python dev/reference/trigger_vocab.py --audit`. Regenerate after an "
        "upstream sync._",
        "",
        ...
        "trigger file on the machine (`dev/reference/trigger_vocab.py`), and their file is "
