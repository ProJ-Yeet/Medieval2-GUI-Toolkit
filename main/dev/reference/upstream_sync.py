"""Track Mylae's m2tw-editor, which has no releases and commits straight to main.
    ...
    python dev/reference/upstream_sync.py status     # what the manifest says today
    python dev/reference/upstream_sync.py triage     # file new upstream files as untriaged
    python dev/reference/upstream_sync.py sync       # what changed since we last looked
    python dev/reference/upstream_sync.py sync --accept   # …and mark it as looked at
...
touched by it. `docs/upstream/PORT_MANIFEST.json` then records what we intend to do with
each of his files:
...
disposition, and writes a dated entry to `docs/upstream/SYNC_LOG.md`. Files he adds that
...
MANIFEST = ROOT / "docs" / "upstream" / "PORT_MANIFEST.json"
SYNC_LOG = ROOT / "docs" / "upstream" / "SYNC_LOG.md"
