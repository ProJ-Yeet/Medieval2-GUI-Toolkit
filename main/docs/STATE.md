...
animation editor opens on a packed action in any mod with packs of its own and
**saves the edit into the pack**, appended under
`animations/edited/<skeleton>/<name>.cas` with the skeleton's entry rewritten
to point its slot there (`animpack.replace_entry`; appending the skeleton again
would lose to the first copy, 82's question 5); **Use it here** puts another
mod's take on a slot into this one, its bytes as they are when the bones match,
carried across by bone name when not; **Bring a skeleton from another mod…**
ports one skeleton with its animations, renamed `<name>_<source>` when the
name is taken, optionally pointing the model on screen at it. All three keep
the mod rebuildable (a loose file and `descr_skeleton.txt`'s line) and are one
Undo each. The port now picks a slot's entry by path and scale. The editor's
loose save says a loose file does not override the pack. `test_animslot` (50).
**In-game checks pending**: an edit saved in the editor playing in a battle,
and a model on a renamed skeleton animating. 82's write-up is in
`ROADMAP_ARCHIVE.md` with 86's. Full suite: 20 of 163 red, the 18 of before
(the installed mods' own data) plus `test_packport`, whose re-measured figure
moved with the port's choice by scale (10 979 slot paths reused, from 11 133;
updated), and `test_factionaudit`'s time limit under six suites at once (66/66
alone).
...
path no slot names, a skeleton name twice), and a compaction that writes the
packs again holding only what is played, the old four kept whole in the mod's
`.ut_compacted/<id>/` until Undo or "Forget the old packs" in the log. On the
Home screen, each mod card's **Animation packs** line. The rule under it: an
animation is its path **and** its scale, a slot plays the copy at its
skeleton's scale, else the smallest rescaled (`animpack.resolve_slot`).
**Measured: all three installs are clean.** DaC's 808 and Reforged's 102 paths
listed more than once are each at a scale of their own, all played, every later
copy an exact rescale of the first (this corrects the morning's note under 82
that Reforged's were dead bytes). Found on the way: 81's port picks a slot's
entry by path alone; on screen it plays the same because of those exact
...
phases"). `unittransfer/animloose.py`: for each skeleton a port adds, a loose
`.cas` per animation it plays (where none is found), an `.evt` per distinct
set of cues, and a `type` block at the end of `descr_skeleton.txt` rendered
from the packed skeleton, its combat settings from the source's own block.
`TransferOptions.keep_rebuildable`, on by default, with a second switch in the
composer; one undo takes it all. The `Version` line is never touched, so it
never starts a rebuild. Proved without the game: `to_packed` packs a `.cas`
the way the engine does, and every ROCSS animation and every tenth DaC
skeleton's round-trip to their entry (one DaC entry, `Knife_Default`, is
built at another scale than it says and cannot); every slot of both mods
round-trips as flags. The working model of a rebuild (written under 82's
questions 2 and 4): only when neither the mod's packs nor vanilla's load,
and one missing file fails the whole rebuild; no installed mod can rebuild, so
84's done-when needs a kit mod that can. `test_animloose` (19).

**82, questions 5 and 6 answered in game, 2026-09-26.** `dev/checks/phase82_kit.py`
put two kits on Steward's Guard in Reforged (four pack files backed up whole):
a second `pack.idx` entry for its standing idle holding `die_forward_2`, and a
loose `.cas` at its walk's path holding `celebrate_1`. The user: "they idle and
walk normally". So **the first of two duplicates plays**, and **a loose `.cas`
does not override a packed path**, even with Reforged's `[io] file_first = true`.
The game's log (`logs/trace.log.txt` in the game folder, rewritten each launch)
has nothing about either; `pack.dat` was untouched, so no rebuild either. Kit
undone, all four files byte for byte. Consequences written into 82, 85 and 86 in
`ROADMAP.md`: 86 saves an edit under a new path and repoints the slot; 85 keeps
the first copy. Left in 82: question 2 and what triggers a rebuild.
...
reforged"). `TransferOptions.bring_animations` (on by default) ports every
body and weapon skeleton a copied model names that the destination's pack
lacks, with Phase 81's engine, renames them in the copied entries when taken,
does the append first and refuses before writing anything, and folds the
appends into the transfer's one undo. `descr_skeleton.txt` is deliberately left
alone until 84 (a rebuild from it could drop pack-only animations).
**Done for real**: DaC EUR's Hobbit Infantry (`EUR_Hobbit_Spear`) into the
...
once more.
**87h is done**, and with it 87 (the user: "finish up 87"): *Export the map
bundle* on the Real world tab (`mapbundle.py`), a zip in Mylae's layout. It
holds the ten layers byte for byte as on disk, `bbox_coords.txt` with the
rotation, and `reference/` with `map_regions.txt` in his columns, the historic
...
vanilla's, for a mod with none), named by 78's table, grouped by family with a
find box, frames, seconds and metres beside each, and read straight out of
`pack.dat` with the packed skeleton's bones; a loose file at the path is
preferred. Each entry's skeleton sets (per mount type) and weapon skeletons
are listed; the slot's impact frame, events and turn limits are marked under
...
2026-09-25, committed and uncut.** A transfer holds a model's skeletons against
the destination's own `skeletons.idx` (`animpack.known_skeletons`; the modeldb
only for a mod with no pack), case-blind, and counts the **weapon skeletons**
too, reported apart with Makanyane's rule (it only shows if the mesh has weight
on the weapon bones). New Health source `skeletons`: ROCSS's own pack lacks
exactly three weapon skeletons, DaC's none. `test_transfer_skeletons` (17).
Full suite: 38 of 148 red, 37 of them red on the commit before too (the
installed mods); the 38th, `test_animedit`, passes alone (it shares the real
undo log with parallel suites). **80 is next.**

**Phase 78 is done - the 687 slots named, and `descr_skeleton.txt` held
against the packs, 2026-09-25, committed and uncut.** `data/skeleton_slots.json`
names 455 slots, every slot any installed skeleton fills, aligned from 678
skeletons whose text and pack agree in count (21 of them in groups whose names
always share a path); `dev/reference/skeleton_slots.py` rebuilds it.
`unittransfer/skelslots.py` reports a mod's text against its packs: DaC is in
step on all 410; ROCSS on 203 of 208 (five weapon skeletons name vanilla's
path where the pack holds ROCSS's copy). `casanim` now reads a path with a
space in it (vanilla's camels, DaC's witch), which it cut short before.
`test_skelslots` (22). No UI. **79 is next.**

**Phase 77 is done - the packs, read, 2026-09-25, committed and uncut.**
`unittransfer/animpack.py` reads `pack.idx`/`.dat` and `skeletons.idx`/`.dat`
straight out of the files, nothing unpacked: an index with every duplicate
kept, one animation as its arrays, one skeleton as its bones and 687 slots
with the combat tables carried as bytes, and a per-folder cache on the four
files' time and size. All 730 skeletons on vanilla, ROCSS and DaC and all six
indexes serialize back byte for byte; every structural check passes on all
three in about 2 s (`test_animpack`, 50). No UI. **78 is next.**

**Cut v2.3.9 and beta 2026-09-25, 2026-09-25, on the user's word ("start
next two phases and then subrelease it").** Everything since v2.3.8: the
unpacked-pack animations, 68, 69, 71's data-zip load, 74, 75, 76 and the strat
texture order go on both lines; 70, 71's campaign zip, 72, 73, 25, 26, 27, the
validator changes (ports on their dock, stacked resources, dense forest,
uneven crossings and Smooth) and the new-campaign header fix are the beta's.
**77 is next.**

**Phase 27 is done - the layer generators, 2026-09-24, committed.** The
Campaign Map's *Generate* tab (under Map): heights from the real world at the
engine's own scale (land only unless asked), ground types from the heights by
bands, climates from the ground types, and rivers, cliffs and volcanoes from
OpenStreetMap drawn as the engine can build them. Each is a plan with a
preview, then one Undo. The two network ones are under the Real world switch,
which is still off. The Köppen and land-cover fetchers were not built; the
archive says why. **77 is next.**
...
`--latest` (latest **v2.4.0**, 2026-09-26). **Campaign-map** -> the **beta
line**, uploaded as a **pre-release** (latest **beta 2026-09-26**). A
...
`master` carries the map ON, and `__version__` says `beta-2026-09-26`
