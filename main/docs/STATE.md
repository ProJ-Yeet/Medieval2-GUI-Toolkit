...
Only the three things a person opens are at the top of the repository: the two
`.bat` files and `README.md`. **Everything else is under `main/`, and `main/` is
now the code root** - it is what `config.PROJECT_ROOT` resolves to, what a test's
`parents[1]` is, and what every path written in this file and in the source is
relative to. So `web/js/core.js` still means `web/js/core.js`; it is just at
`main/web/js/core.js` on disk.

| | |
|---|---|
| `main/app.py`, `main/transfer_cli.py` | the two entry points |
| `main/unittransfer/`, `main/web/` | unchanged, one level down |
| `main/vendor/nvtt/` | was `tools/nvtt/`. The one binary that ships |
| `main/docs/` | this file, ROADMAP, and both archives |
| `main/docs/releases/` | the release notes (was half of `merge/`) |
| `main/docs/upstream/` | the reference audits, port manifest, sync log (the other half) |
| `main/docs/images/` | the README's screenshots, taken by `dev/docs/screenshots.py` |
| `main/dev/` | scripts for working ON the toolkit, grouped by what they do: `release/`, `reference/`, `docs/`, `checks/`, `diagnose/`. **None of it ships.** See `dev/README.md` |

The launcher works in both layouts: it steps into `main/` when `main/app.py` is
there and runs from beside itself when it is not, because the same file is what
goes into the release zip, where everything is flat.
...
`docs/releases/RELEASE_BETA_2026_09_06.md` and the zip is on GitHub as a pre-release. Its
...
Run `python dev/reference/upstream_sync.py sync` first, as before every sub-phase.
...
- `web/js/core.js`, `web/index.html`, `dev/checks/prose_check.py` - the menu hints as
...
`docs/upstream/SYNC_LOG.md`; the detail from the sync before it (what
...
`docs/upstream/PORT_MANIFEST.json` is authoritative. It triages all 310 upstream files:
128 port-concept, 90 skip, 71 out-of-scope, 21 audit, none untriaged. The
port-concept set is what `docs/upstream/REFERENCE_GAPS.md` measures against.

## Open questions for the user
- ~~`docs/upstream/REFERENCE_GAPS.md` needs prioritising.~~ **Answered 2026-09-05:**
...
- 2026-09-05: **Future work gets a phase number; a version number is assigned
  ...
  worked, and `docs/upstream/PORT_MANIFEST.json`'s `notes` were updated in the same
