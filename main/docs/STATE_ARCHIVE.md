# STATE ARCHIVE - the session log

**This file is history.** One section per session, newest first, saying what
that session did and what it found out. It is not the project's state: that is
`STATE.md`, which holds only where things stand right now and is rewritten
rather than appended to.

Read a section here when you need to know *why* a module does something the
obvious way would not, and the answer is not in `ROADMAP_ARCHIVE.md`. Everything
below was written at the end of the session it describes and was true then.
...
| You want | Read |
|---|---|
| where the project is now | `STATE.md` |
| the next thing to do | `ROADMAP.md`, Backlog |
...
right for all three. `dev/checks/prose_check.py` grew `ANY_EM`, which scans whole
...
default (`BUNDLED_DIRS` in `dev/release/build_release.py`), a missing `vanilla_ui/` stops the
...
Notes: `docs/releases/RELEASE_2_1_5.md`. `tests/test_bmdb.py` 104 → 143 checks.
...
Notes: `docs/releases/RELEASE_2_1_4.md`. New: `tests/test_back_button.py` (21),
...
`docs/releases/RELEASE_2_1_3.md`. New: `tests/test_stratmap.py` (43),
...
field itself. Notes: `docs/releases/RELEASE_2_1_2.md`. New: `tests/test_modflags.py`
...
Notes: `docs/releases/RELEASE_2_1_1.md`. New: `tests/test_broken_mod_files.py` (22).
...
released with the portable zip. Notes: `docs/releases/RELEASE_2_1_0.md`.
...
**14j is a SUBRELEASE of its own** (`v2.0.1`, `docs/releases/RELEASE_2_0_1.md`), unlike
...
2.0.0, and `docs/releases/RELEASE_2_0_0.md` carries the new work as a "The correction
...
texture path. `dev/diagnose/meshdump.py` is the debugging tool: `--sweep <folder>` over
...
`STATE.md`'s Decisions list is append-only and had reached 53 entries. What
follows is everything dated before 2026-09-05, newest first, moved here on
2026-09-05.

Two of them graduated rather than being archived and now live in `ROADMAP.md`'s
Locked decisions, where a session will actually read them: **a rule with no
evidence reports nothing**, and **a baseline shows and stops blocking; it never
...
- 2026-08-19: **A measurement is fixed before the thing it measures.** 14g opened
  ...
  `dev/checks/prose_check.py` failing to stitch the `+` continuations its own
...
# The last per-phase status table, and the notes under it

`STATE.md` used to carry a Phase status row for every phase ever finished, each
with a paragraph of detail in it, plus an In-progress section that had not been
true since Phase 14. Both are below as they stood on 2026-09-05, kept because
several rows carry a measurement that is not written down anywhere else.
...
| 13 - EDU + Sounds audit | done | `docs/upstream/audit-edu-sounds.md`; measured over 1756 real units; **nothing adopted from their code**, banners rederived from the mod's own file, two silent rewrites of ours fixed |
| EDB corpus follow-up | done | `#` annotation lines no longer read as capabilities (3 parsers + regression case); the `plugins` and upgrade-clause sweeps no longer depend on which mods are installed; `docs/upstream/audit-edb.md` corrected |
| 15a - the model decoder | **done** | `unittransfer/mesh.py` + `dev/diagnose/meshdump.py`. A `.mesh` is a **boost::serialization archive**, reverse-engineered from the files: neither the Blender addon (it shells out to IWTE) nor upstream's `casCodec.js` (its spec matches no real file) could be ported. **4,700 of 4,702 models decode** (the 2 are multi-model sky domes, refused by name), proven by reaching the bone table. Two vertex formats: skinned packs normals to bytes, static writes floats. `.cas` NOT decoded - a whole 3ds-max scene format, handed to 16e. `test_mesh` 34/34 |
...
| 14g - the second prose sweep | done | 21 clause-joining dashes → **0**, four documented keeps. 6 of the old 115 hits' causes were defects in `dev/checks/prose_check.py` itself, not in the writing |
...
| Release check (14h) | done | `docs/upstream/audit-codebase-2.md`. A BOM cost the EDU and factions parsers their first record SILENTLY (DaC really lost a faction); mixed line endings no longer normalised; cache invalidation derived from `Mod`; the 56 "missing ancillary picture" findings were ours, not the mod's; three suites that could not run, run. **52/52 green.** |
...
in `docs/upstream/audit-codebase-2.md` §3.
...
`docs/upstream/PORT_MANIFEST.json` is authoritative; all 12 phase-13 files carry their
