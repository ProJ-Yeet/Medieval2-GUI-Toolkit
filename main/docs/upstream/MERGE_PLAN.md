...
`docs/upstream/STATE.json` and `docs/upstream/PORT_MANIFEST.json`, decisions in `docs/upstream/decisions/`.
Start any session by reading `docs/upstream/STATE.json`.
...
2. Record the outcome in `docs/upstream/decisions/0001-licensing.md`.
...
2. `docs/upstream/STATE.json` records `upstream.reviewed_sha` - the last commit we have
   triaged (not merged; *triaged*).
3. `docs/upstream/PORT_MANIFEST.json` holds one row per upstream file:
   `{path, sha_at_review, disposition, target, status, notes, tests}`.
4. `dev/reference/upstream_sync.py` - the workhorse:
   ...
   - writes a dated entry to `docs/upstream/SYNC_LOG.md` and bumps `reviewed_sha`
...
one commit. `docs/upstream/STATE.json` names the current phase and the next pending item,
so any session resumes by reading it. No phase depends on a later one.

| # | Phase | Output | Gate to start |
|---|---|---|---|
| **0** | **Tracking + legal infra** | upstream remote fetched, `docs/upstream/STATE.json`, empty manifest, `dev/reference/upstream_sync.py`, licence question sent | - |
...
| **8** | **Packaging** | `dev/release/build_release.py --with-studio` bundles prebuilt Studio assets; `Unit Transfer.bat` gains a Studio toggle; release zip size budget checked | Phase 6 |
