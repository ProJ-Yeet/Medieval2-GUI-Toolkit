...
pyflakes over `unittransfer/`, `tools/`, `app.py` and `dev/release/build_release.py` is clean
apart from one deliberate probe. Removed: 8 unused imports, 3 unused locals and an
f-string with no placeholders, plus one more in `dev/reference/trigger_vocab.py` that
