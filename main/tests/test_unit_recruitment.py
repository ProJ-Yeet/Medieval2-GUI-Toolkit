...
# The package lives one level up from tests/. Python puts the SCRIPT's own
# folder on sys.path, not the working directory, so without this line the
# suite only imports when it happens to be run from the right place - and
# the README says each of these is runnable on its own.
